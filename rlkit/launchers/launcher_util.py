...
def run_experiment(
    ...
    try:
        ...
        multiworld_path = multiworld.__file__
        rlkit_path = rlkit.__file__
        multiworld_repo = git.Repo(multiworld_path[:-23]) # multiworld's path len should be -23 and rlkit is -18
        rlkit_repo = git.Repo(rlkit_path[:-18])
        multiworld_git = multiworld_repo.git
        rlkit_git = rlkit_repo.git
        multiworld_commit = multiworld_git.log()[0:47] # return the repositories commit info
        rlkit_commit = rlkit_git.log()[0:47]
        print('\nThe multiworld version: '+multiworld_commit)
        print('\nThe rlkit version: '+rlkit_commit)
